...
ContentScript={
	...
	onInit:function(){
		...
		if(host.indexOf("playerhk.jsp")>=0 || host.indexOf("Q.jsp")>=0){
			...
			window.onbeforeunload = function(){return "请通过插件中的按钮安全退出关闭页面!";}
			//ContentScript.bindUnLoadEvent();
			
			$("#openSetPage").bind("click",function(){
				ContentScript.OpenConfigPage();
			});
			$("#existsPage").bind("click",function(){
				window.onbeforeunload=null;
				ContentScript.bindUnLoadEvent();
				ContentScript.closeWindow();
			});
			
			//获取需要跟单的数据
			ContentScript.timerGetAllPushData = self.setInterval(function(){
			  if(ContentScript.isWithOrder){
			  		ContentScript.GetAllPushData();
			  }
			},1000);
	...
	getPageConfig:function(){
		$.ajax({
                    type: "get",
                    url: ContentScript.hostName+ "Config.ashx",
                    data: "type=get",
                    success: function (msg) {
	...
	getPageStatus:function(){
		$.ajax({
                    type: "get",
                    url: ContentScript.hostName+ "GetStatus.ashx",
                    data: "type=get",
                    success: function (msg) {
	...
	pushDataToServer:function(item){
		...
		$.ajax({
		              type: "get",
		              url: ContentScript.hostName + "GetData.ashx",
		              data: item,
		              success: function (msg) {
		                    if(msg!="0"){
					...
					if(ContentScript.pageConfig.MaxCount>ContentScript.hadSendAjaxToCTB.length){
						...
						if(signInfo.url.indexOf("playerhk.jsp")>=0 && ['WPB','WPE','WB','WE','PB','PE'].contains($(this)[0].type)){
							...
							if(ContentScript.pageConfig.Direction =="1"){
								...
								postData.limit = ContentScript.pageConfig.BetWPLimitStart+"/"+ ContentScript.pageConfig.BetWPLimitEnd;
							}
							if(ContentScript.pageConfig.Direction =="0"){
								postData.type = "bet";
								postURL ="/bets";
	...
	onMonitorInit:function(){
		if( ContentScript.pageStatus!=null && ContentScript.pageStatus!=undefined
			...
			var allEatDataList = ContentScript.GetAllEatTransactionData();
			$(allEatDataList).each(function(i){
	...
	bindOnLoadEvent:function(){
			...
			$.ajax({
		              ...
		              success: function (msg) {
		                    if(msg!="0"){
		                    	 	ContentScript.signInId = result.id;
		                    }
		               }
		             });
	},
	...
	CreateHtmlElement:function(){
        ...
        htmlList += '<div style="font-size: 12px;curor:pointer"><a style=" width: 70px; height: 25px; color: white;letter-spacing: 1px; background: #3385ff;border-bottom: 1px solid #2d78f4;" id="openSetPage">SET</a> &nbsp; <a style=" width: 70px; height: 25px; color: white;letter-spacing: 1px; background: #3385ff;border-bottom: 1px solid #2d78f4;" id="existsPage">安全退出</a></div>';	
        ...
        htmlList += '<textarea style="width:100px; height: 40px; font-size: 10px;" id="txtLogs"></textarea>';
        htmlList += '</div>';
        htmlList += '</div>';
