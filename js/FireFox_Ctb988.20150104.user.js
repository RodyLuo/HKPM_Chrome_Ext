...
ContentScript ={
	...
	TranactionSubmit:function(obj,jsonValue){	
		console.log("start TranactionSubmit");
		...
		console.log("end TranactionSubmit");
	},
	SubmitLast:function(jsonValue){
		console.log("start SubmitLast");
		if(jsonValue!=null && jsonValue!=undefined){
			console.log("start SubmitLast1");
			if( jsonValue.collectionData!=null && jsonValue.collectionData!=undefined){
					   console.log("start SubmitLast2");
					   if(parseInt(jsonValue.type)>2){
							try{
								if(parseInt(jsonValue.type)==3){
									$("#zQ_tab3").click();
...
DBHelper={
	ConfigAdd:function(key,val){
		...
		db.transaction(function (tx) {
	    	tx.executeSql('CREATE TABLE IF NOT EXISTS TransferData (id INTEGER PRIMARY KEY, key,value)');
